  ...
  return (
    <section ref={sectionRef} className="bg-[#0C0C0C] pt-24 sm:pt-32 md:pt-40 pb-10 overflow-hidden">
      <div className="flex flex-col gap-3">
        ...
        >
          {tripleImages(imagesRow1).map((img, i) => (
            <img key={i} src={img} alt="" className="w-[420px] h-[270px] rounded-2xl object-cover flex-shrink-0" loading="lazy" />
          ))}
        </div>
        ...
        <div 
          ...
          style={{ transform: `translateX(${-(offset - 200)}px)`, willChange: 'transform' }}
        >
          {tripleImages(imagesRow2).map((img, i) => (
            <img key={i} src={img} alt="" className="w-[420px] h-[270px] rounded-2xl object-cover flex-shrink-0" loading="lazy" />
          ))}
        </div>
      </div>
